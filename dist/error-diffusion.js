 ...
 * So it returns 51
 */
function findNearestLevel(value, levels) {
 ...
 */
function applyErrorDiffusion(imageData, width, height, levels) {
 ...
 */
async function createSideBySideImage(originalBuffer, encodedBuffer, width, height) {
    const padding = 20;
    // Simply place images side by side using extend
    const composite = await (0, sharp_1.default)(originalBuffer)
        .extend({
        right: padding + width,
...
async function main() {
    try {
        ...
        const image = (0, sharp_1.default)(filename).grayscale();
        ...
        const originalBuffer = await (0, sharp_1.default)(data, {
            raw: {
                width: width,
                height: height,
                channels: 1
        ...
        const encodedBuffer = await (0, sharp_1.default)(encodedData, {
            raw: {
                width: width,
                height: height,
                channels: 1
        ...
        await (0, sharp_1.default)(encodedBuffer).toFile(outputFilename);
        console.log(`✓ Encoded image saved as: ${outputFilename}`);
        // Create and save side-by-side comparison
        const comparisonBuffer = await createSideBySideImage(originalBuffer, encodedBuffer, width, height);
        const comparisonFilename = 'comparison.png';
        await (0, sharp_1.default)(comparisonBuffer).toFile(comparisonFilename);
